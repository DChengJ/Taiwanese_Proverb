...
tf.app.flags.DEFINE_string('wv_path', 'word2vec/100.model.bin', 'path to directory')
...
def output_word2vec(word2vec):
    #print(len(word2vec.vocab.keys()))
    with open('word2vec/wiki.skip.cwe.100.txt', 'w', encoding='utf-8') as f:
        for word in word2vec.vocab.keys():
            f.write(word + '\t')
            f.write('\t'.join(str(v) for v in word2vec[word]))
            f.write('\n')
