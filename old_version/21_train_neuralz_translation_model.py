...
train = load_dataset('./crossvalidation/newProverb2_jieba_v2_3-train.pkl')
test = load_dataset('./crossvalidation/newProverb2_jieba_v2_3-test.pkl')
...
for line in lines:
    ...
    for word in words:
        if word not in index_dict:
            disappear.append(line)
            break
for line in disappear:
    print(line)
...
for line in dd:
    ...
    for word in line:
        ...
        if word not in index_dict and word not in disappear:
            disappear.append(word)
            i+=1
            break

with open('nonexistent.txt', 'w') as f:
    for word in disappear:
        f.write(word + '\n')
